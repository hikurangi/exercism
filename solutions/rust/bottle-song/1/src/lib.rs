const SECOND_LINE: &'static str = "And if one green bottle should accidentally fall,\n";
...
fn recite_rec(verses: String, current_bottles: u32, take_down: u32) -> String {
    ...
    let current_bottle_count_text = match current_bottles {
        ...
        _ => panic!(
            "Invalid starting bottle count supplied: {}",
            current_bottles
        ),
    };
    ...
    let first_line = format!(
        "{count} green bottle{current_plural_suffix} hanging on the wall,\n",
        count = current_bottle_count_text,
        current_plural_suffix = current_plural_suffix
    );
    let final_line = format!(
        "There'll be {count} green bottle{next_plural_suffix} hanging on the wall.",
        count = next_bottle_count_text.to_lowercase(),
        next_plural_suffix = next_plural_suffix
    );
