...
pub fn recite(start_bottles: u32, take_down: u32) -> String {
    // This exercise doesn't let us return a Result. The possible fn inputs can be wildly invalid, so here we are.
    assert!(start_bottles >= take_down && start_bottles > 0 && take_down > 0);
    (0_u32..take_down).fold(String::new(), |verses, i| {
