...
public class Allergies
{
    ...
    private Allergen[] ListAllergens(int mask, HashSet<Allergen> allergens) {
        ...
        var largestFitInt = _values.Where(v => v <= mask).Max();
        var largestFitAllergen = (Allergen)largestFitInt;

        allergens.Add(largestFitAllergen);

        return ListAllergens(mask - largestFitInt, allergens);
    }
